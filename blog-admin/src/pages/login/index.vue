    ...
    <div>
        ...
        <div class="sub-main-w3">
            <form action="javascript:;">
                ...
                <!-- //checkbox -->
                <input type="submit" value="登 陆" id="submit" @click="Register"/>
            </form>
        </div>
        <!-- copyright -->
        <div class="footer">
...
// const canvasBack: any = () => import('../register/canvas.js')
import canvasBack from './../register/canvas.js'
export default {
    name: 'login',
    setup() {
        ...
        const Register = () => {
            if(search.username&&search.password){
                proxy.$api('userLogin', search).then((res: any) => {
                    ElMessage.success(res.message)
                    proxy.$store.commit('user/setUserInfo',res)
                    proxy.$router.push('/')
                })
            }
        }
